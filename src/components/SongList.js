...
export default function SongList(props) {
    ...
    useEffect(() => {
        //====> SAHIFA LOAD BO'LGANDA HAMMA MP3 FAYLLARNING DURATION VAQTINI OLIB BERADI
        getDurationsOfAudios();
    }, []);

    useEffect(() => {
        //====> LISTDAGI QO'SHIQLARDAN BIRINI TANLAGANDA ULARNING STILIGA MANIPULYATSIYA QILISH UCHUN
        const titleItems = document.querySelectorAll(".song__title");
        titleItems.forEach(item => {
            //BIROR QO'SHIQ TANLANGANDA TITLE NI RANGINI O'ZGARTIRADI
    ...
    useEffect(() => {
        ...
        indexItems.forEach(item => {
            //BIROR QO'SHIQ TANLANGANDA QO'SHIQNING TARTIB RAQAMINI ANIMATSIYAGA YOKI RANGINI O'ZGARTIRADI
            item.classList.remove("playing-now");
            item.innerHTML = item.id;
            if (item.id === audioRef?.current?.id) {
                item.classList.add("playing-now");
                if (isPlaying) {
                    //AGAR TANLANGAN PAYTDA QO'SHIQ IJRO ETILIB TURGAN BO'LSA ANIMATSIYA CHIQADI
                    item.innerHTML = "<img src='../image/animated-equalizer.gif' />";
                } else {
                    //AGAR TANLANGAN PAYTDA QO'SHIQ PAUZA BO'LSA TARTIB RAQAMNI KO'RSATADI.
                    item.innerHTML = item.id;
                }
            }
        });
    ...
    const getDurationsOfAudios = () => {
        //HAR BIR MP3 NING 'duration' QIYMATINI ANIQLAB APPJS DAGI QO'SHIQ OBYEKTIGA QO'SHIB QO'YADI
    ...
    const calculateTime = secs => {
        //YAXLITLANGAN SEKUNDLARNI 'MINUT:SEKUND' KO'RINISHIDA QAYTARADI
    ...
    const chooseSong = index => {
        //RO'YXATDAGI QO'SHIQNI BOSGANDA PLAYERGA SHU QO'SHIQNI UZATADI.
        props.setCurrentSongIndex(index);
        setIsPlaying(true);
        activeBg(index);
    };

    const activeBg = index => {
        //QO'SHIQLAR RO'YXATIDAGI BOSILGAN QO'SHIQNING ORQA FONINI O'ZGARTIRADI
