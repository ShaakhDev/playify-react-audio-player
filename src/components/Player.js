...
function Player(props) {
    ...
    useEffect(() => {
        //AUDIONING UMUMIY DAVOMIYLIGINI YAXLITLAB 'duration' GA UZATADI
        setTimeout(() => {
    ...
    const changeRange = () => {
        //QO'SHIQNI ISTALGAN JOYIDAN IJRO QILISH.
    ...
    const whilePlaying = () => {
        ...
        if (audioRef.current.currentTime === audioRef.current.duration) {
            //BITTA QO'SHIQ TUGAGACH AVTOMATIK RAVISHDA KEYINGI QO'SHIQQA O'TISH
            SkipSong();
        }
    };
    ...
    const changeVolumeRange = () => {
        ...
        volumeProgressRef.current.style.setProperty(
            "--volume-before-width",
            `${volumeProgressRef.current.value}%`
        );
        ...
        <div className="player">
            ...
            {isMobile && (
                <button className="btn collapse-btn">
                    {clickCollapseBtn ? (
                        <ChevronDown onClick={handleCollapseBtnClick} />
                    ) : (
                        <ChevronUp onClick={handleCollapseBtnClick} />
                    )}
            ...
            <div className="player__progressbar">
                ...
                {/* volume icon  */}
                {isMute ? (
                    <SoundOff className="volume-icon" strokeWidth={1} />
                ) : (
                    <SoundOn className="volume-icon" strokeWidth={1} />
                )}
                {/* volume progressbar */}
                <div className="volume">
                    <input
                        ...
                        defaultValue={50}
                        max={100}
                        className="progressbar volume-progressbar"
                    />
                </div>
            </div>
        </div>
